    ...
    <div class="text-center q-mx-auto" style="max-width: 600px">
      ...
      <div class="q-gutter-y-md q-mt-md">
        <q-btn
          ...
          :label="
            connected
              ? t('Welcome.nostr.connected')
              : t('Welcome.nostr.connect')
          "
          @click="connectNip07"
          :disable="connected"
          :loading="connecting"
        ...
        <div v-else-if="!nip07Detected" class="text-caption">
          ...
          <div>
            {{ t("Welcome.nostr.installBrowser", { browser: browserLabel }) }}
          </div>
          <ul class="q-mt-xs">
        ...
        <q-btn
          color="primary"
          :label="t('Welcome.nostr.generate')"
          @click="generate"
        />
        <q-form @submit.prevent="importKey">
          <q-input
            v-model="nsec"
            :label="t('Welcome.nostr.importPlaceholder')"
            autocomplete="off"
          />
  ...
  try {
    ...
    if (retryable && allowRetry) {
      const delayMs = Math.min(
        750 * Math.max(1, nostr.nip07RetryAttempts || 1),
        4000,
      );
...
async function generate() {
  error.value = "";
  try {
    await nostr.initWalletSeedPrivateKeySigner({ skipRelayConnect: true });
    welcome.nostrSetupCompleted = true;
    npub.value = nostr.npub;
...
async function importKey() {
  ...
  try {
    await nostr.initPrivateKeySigner(nsecToUse, { skipRelayConnect: true });
    welcome.nostrSetupCompleted = true;
    npub.value = nostr.npub;
    backupNsec.value = nostr.activePrivateKeyNsec;
    showBackup.value = true;
