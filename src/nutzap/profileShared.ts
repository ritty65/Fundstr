...
const MEDIA_EXTENSION_TYPE_MAP: Record<
  string,
  Exclude<TierMedia["type"], undefined>
> = {
  jpg: "image",
  jpeg: "image",
  png: "image",
...
const MIME_PREFIX_TYPE_MAP: Array<{
  prefix: string;
  type: Exclude<TierMedia["type"], undefined>;
}> = [
...
export function inferTierMediaType(
  url: string,
  providedType?: string,
): TierMedia["type"] {
  ...
  if (normalizedProvided) {
    if (
      normalizedProvided === "image" ||
      normalizedProvided === "video" ||
      normalizedProvided === "audio"
    ) {
      return normalizedProvided;
    }
    const mimeMatch = MIME_PREFIX_TYPE_MAP.find(({ prefix }) =>
...
function selectLatestByKey<T extends ReplaceableLike>(
  ...
  for (const event of events) {
    ...
    if (
      !current ||
      ((event as any).created_at ?? 0) > ((current as any).created_at ?? 0)
    ) {
      map.set(key, event);
    }
  }
  ...
  if (trimmed.toLowerCase().startsWith("npub")) {
    try {
      ...
      if (typeof data === "string") {
        hex = data;
      } else if (ArrayBuffer.isView(data as object)) {
        hex = Array.from(data as Uint8Array)
...
export function pickLatestReplaceable<T extends ReplaceableLike>(
  events: readonly T[],
): T | null {
...
export function pickLatestParamReplaceable<T extends ReplaceableLike>(
  events: readonly T[],
): T | null {
  return selectLatestByKey(events, (event) => {
    const kind = (event as any).kind;
    ...
    return typeof kind === "number" && pubkey
      ? `${kind}:${pubkey}:${dValue}`
      : "";
  });
}
...
function normalizeRawTier(raw: RawTier): Tier | null {
  ...
  const frequency: Tier["frequency"] = [
    "one_time",
    "monthly",
    "yearly",
  ].includes(raw.frequency as Tier["frequency"])
