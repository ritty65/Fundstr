...
- `scripts/maintenance/sync-hostinger-profiles-from-relay.py`
- `scripts/maintenance/sync-hostinger-creators-from-relay.py`

These tools:

- read source data directly from the local VPS Strfry database via `strfry scan`
- write directly into Hostinger MySQL over IPv4 TCP
- do not touch the relay
- do not use public HTTP write endpoints
- support `--dry-run`

Why relay scan instead of the old SQLite export path:

- `phonebook_seed.sqlite` is stale compared with the current Hostinger `profiles` table
- `nutzap.sqlite` contains fresh `nutzap_profile`/`nutzap_tier` rows, but its `creators` table is empty
- the local relay database is the cleanest current source of truth for fresh events
...
```bash
python3 scripts/maintenance/sync-hostinger-profiles-from-relay.py --dry-run --limit 20 --lookback-days 60
```
...
```bash
python3 scripts/maintenance/sync-hostinger-creators-from-relay.py --dry-run --limit 10 --lookback-days 3650
```
...
```bash
python3 scripts/maintenance/sync-hostinger-profiles-from-relay.py --limit 100 --batch-size 50 --lookback-days 60
```

Then creators:

```bash
python3 scripts/maintenance/sync-hostinger-creators-from-relay.py --limit 20 --batch-size 20 --lookback-days 3650
```
...
- retire the obsolete `seed-profiles-from-creators.mjs` pipeline if the relay-backed path proves stable
