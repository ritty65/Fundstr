...
Fundstr — Quasar/Vite PWA app (pnpm). Staging auto-deploys on every push to `Develop2`.

## Setup & daily commands

- Install deps: `pnpm install`
- Dev server: `pnpm dev`
- Build (PWA): `pnpm dlx @quasar/cli build -m pwa`
- Tests: `pnpm test` # (wire up Vitest if needed)
- Lint: `pnpm lint` # if ESLint configured

## CI/CD

- **Staging**: push to `Develop2` → GitHub Action builds & rsyncs to Hostinger → https://staging.fundstr.me/
...
- Do release work from a fresh clean worktree based on `origin/Develop2` or `origin/main`, not from stale sandbox folders.

## Definition of Done

- Local build passes.
- `pnpm test` (if tests exist) passes.
- After push to `Develop2`, staging loads without console errors; `deploy.txt` updated.
- `find_profiles.php` returns JSON on staging/production after deploy.
