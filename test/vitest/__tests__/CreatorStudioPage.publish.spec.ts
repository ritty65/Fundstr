...
const creatorStudioStubs = {
  ...
  CreatorStudioPreviewCard: { template: "<div />" },
};
...
it("preserves local profile and tier drafts when no relay snapshot exists yet", async () => {
  ...
  vmAny.displayName = "Draft Creator";
  ...
  vmAny.applyProfileEvent(null);
  vmAny.applyTiersEvent(null);
  await wrapper.vm.$nextTick();

  expect(vmAny.displayName).toBe("Draft Creator");
  expect(vmAny.mintList).toEqual(["https://mint.example"]);
  expect(vmAny.tiers).toHaveLength(1);
...
vi.mock("src/nutzap/useNutzapRelayTelemetry", () => ({
  useNutzapRelayTelemetry: () => {
    ...
    return {
      ...
      publishEventToRelay: (...args: any[]) =>
        state.publishEventToRelayMock(...args),
...
vi.mock("src/nutzap/profileRepublish", () => ({
  maybeRepublishNutzapProfile: (...args: any[]) =>
    maybeRepublishNutzapProfileMock(...args),
}));
...
vi.mock("../../../src/nutzap/relayPublishing", async () => {
  ...
  return {
    ...actual,
    publishTiers: (...args: any[]) =>
      ensureShared().publishTiersToRelayMock(...args),
    publishNostrEvent: (...args: any[]) =>
      ensureShared().publishNostrEventMock(...args),
    ensureFundstrRelayClient: (...args: any[]) =>
      ensureShared().ensureRelayClientMock(...args),
  };
});
...
describe("CreatorStudioPage publishAll fallback", () => {
  it("logs relay telemetry after successful publish", async () => {
    ...
    state.publishTiersToRelayMock.mockImplementation(
      async (_tiers, _kind, options) => {
        const ack = options?.send
          ? await options.send(tierEvent)
          : {
              id: tierEvent.id,
              accepted: true,
              via: "http" as const,
    ...
    state.publishNostrEventMock.mockImplementation(
      async (_template, options) => {
        const ack = options?.send
          ? await options.send(profileEvent)
          : {
              id: profileEvent.id,
              accepted: true,
              via: "http" as const,
  ...
  it("falls back to HTTP when relay publisher rejects", async () => {
    ...
    state.publishEventToRelayMock.mockRejectedValue(
      new Error("Relay socket unavailable"),
    );
    ...
    const tierCallKinds = state.publishTiersToRelayMock.mock.calls.map(
      (call) => call[1],
    );
    ...
    expect(typeof state.publishTiersToRelayMock.mock.calls[0][2]?.send).toBe(
      "function",
    );
    ...
    expect(typeof state.publishTiersToRelayMock.mock.calls[2][2]?.send).toBe(
      "function",
    );
    ...
    expect((wrapper.vm as any).diagnosticsAttention.detail).toContain(
      "HTTP fallback",
    );
    expect(notifySuccessMock).toHaveBeenCalledTimes(1);
    expect(notifyErrorMock).not.toHaveBeenCalled();
  ...
  it("allows publishing with relay warning by relying on HTTP fallback", async () => {
    ...
    expect(state.publishTiersToRelayMock).toHaveBeenCalledTimes(4);
    const tierKinds = state.publishTiersToRelayMock.mock.calls.map(
      (call) => call[1],
    );
...
describe("CreatorStudioPage auto republish fallback", () => {
  it("auto republish succeeds via HTTP when the websocket publish times out", async () => {
    ...
    expect(fallbackResult?.ack?.message).toContain(
      "Timed out waiting for relay OK",
    );
...
describe("CreatorStudio publish structured clone safety", () => {
  it("sanitizes reactive profile templates before signing and sending", async () => {
    ...
    try {
      const result = await actualRelayPublishing.publishNostrEvent(template, {
        send: sendMock,
      });
  ...
  it("sanitizes HTTP fallback publishes with reactive templates", async () => {
    ...
    const fetchMock = vi.fn(async (_input: any, init?: any) => {
      ...
      return {
        ...
        status: 200,
        text: async () =>
          JSON.stringify({ id: payload?.id ?? "fallback-id", accepted: true }),
