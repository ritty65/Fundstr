...
vi.mock("quasar", () => {
  ...
  const QForm = defineComponent({
    ...
    setup(_, { emit, slots }) {
      ...
      return () =>
        h("form", { onSubmit }, slots.default ? slots.default() : []);
    },
  });
  ...
  const QDialog = defineComponent({
    ...
    setup(props, { slots }) {
      return () =>
        props.modelValue
          ? h("div", { class: "dialog" }, slots.default ? slots.default() : [])
          : null;
    },
  });
  ...
  const QCard = defineComponent({
    ...
    setup(_, { slots }) {
      return () =>
        h("div", { class: "card" }, slots.default ? slots.default() : []);
    },
  });
  const QCardSection = defineComponent({
    name: "QCardSection",
    setup(_, { slots }) {
      return () => h("section", slots.default ? slots.default() : []);
    },
  });
  const QCardActions = defineComponent({
    name: "QCardActions",
    setup(_, { slots }) {
      return () => h("footer", slots.default ? slots.default() : []);
    },
  });
  const QLinearProgress = defineComponent({
    name: "QLinearProgress",
    setup() {
      return () => h("div", { class: "linear-progress" });
    },
  });
  ...
  const QList = defineComponent({
    name: "QList",
    setup(_, { slots }) {
      return () => h("ul", slots.default ? slots.default() : []);
    },
  });
  ...
  const QExpansionItem = defineComponent({
    name: "QExpansionItem",
    setup(_, { slots }) {
      return () => h("div", slots.default ? slots.default() : []);
    },
  ...
  const QPage = defineComponent({
    name: "QPage",
    setup(_, { slots }) {
      return () =>
        h("div", { class: "q-page" }, slots.default ? slots.default() : []);
    },
  });
...
const componentStubs = {
  ...
  ThemeToggle: defineComponent({
    name: "ThemeToggle",
    template: '<div class="theme-toggle"></div>',
  }),
  ...
  NostrBackupDialog: defineComponent({
    ...
    props: {
      modelValue: { type: Boolean, default: false },
      nsec: { type: String, default: "" },
    },
    emits: ["update:modelValue"],
    setup(props) {
      return () =>
        props.modelValue
          ? h("div", { class: "backup-dialog" }, props.nsec)
          : null;
    },
  }),
  WelcomeSlideFeatures: defineComponent({
    name: "WelcomeSlideFeatures",
    template: "<div>features</div>",
  }),
  WelcomeSlidePwa: defineComponent({
    name: "WelcomeSlidePwa",
    template: "<div>pwa</div>",
  }),
  WelcomeSlideBackup: defineComponent({
    name: "WelcomeSlideBackup",
    template: "<div>backup</div>",
  }),
  WelcomeSlideMints: defineComponent({
    name: "WelcomeSlideMints",
    template: "<div>mints</div>",
  }),
  WelcomeSlideTerms: defineComponent({
    name: "WelcomeSlideTerms",
    template: "<div>terms</div>",
  }),
  WelcomeSlideFinish: defineComponent({
    name: "WelcomeSlideFinish",
    emits: ["open-wallet"],
    template: "<div>finish</div>",
  }),
};
...
beforeEach(() => {
  ...
  nostrStore = reactive({
    ...
    npub: TEST_NIP07_NPUB,
    hasIdentity: true,
    privKeyHex: "",
    nip07SignerAvailable: ref(true),
    nip07LastFailureCause: undefined,
  ...
  vaultStore = reactive({
    ...
    setEncryptionKeyFromPin: vi.fn().mockImplementation(async () => {
      vaultStore.isUnlocked = true;
    }),
    unlockWithPin: vi.fn().mockImplementation(async () => {
      vaultStore.isUnlocked = true;
    }),
  });
...
describe("WelcomeSlideNostr", () => {
  it("connects via extension and marks setup complete", async () => {
    ...
    const connectBtn = wrapper
      .findAll("button")
      .find((btn) => btn.text().includes("Welcome.nostr.connect"));
    ...
    expect(quasarNotify).toHaveBeenCalledWith(
      expect.objectContaining({
        message: "Welcome.nostr.connected",
        type: "positive",
      }),
    );
  ...
  it("generates secret and uses loginWithSecret", async () => {
    ...
    const generateBtn = wrapper
      .findAll("button")
      .find((btn) => btn.text().includes("Welcome.nostr.generate"));
    ...
    expect(welcomeStore.nostrSetupCompleted).toBe(true);
    expect(wrapper.text()).toContain(TEST_NPUB);
  ...
  it("shows error when extension unavailable", async () => {
    ...
    expect(quasarNotify).toHaveBeenCalledWith(
      expect.objectContaining({
        message: "Welcome.nostr.errorMissingExtension",
        type: "negative",
      }),
    );
    expect((wrapper.vm as any).error).toBe(
      "Welcome.nostr.errorMissingExtension",
    );
    delete (globalThis as any).nostr;
  });
});
...
describe("NostrSetupWizard", () => {
  ...
  it("walks through private key flow and connects", async () => {
    ...
    await keyInput!.setValue(TEST_NSEC);
    const pinInputs = step1.findAll('input[type="password"]');
    await pinInputs[0].setValue("1234");
    await pinInputs[1].setValue("1234");

    const nextFromKey = step1
      .findAll("button")
    ...
    expect(nostrStore.initPrivateKeySigner).toHaveBeenCalledWith(TEST_NSEC);
    ...
    const finishBtn = step3
      .findAll("button")
      .find((btn) => btn.text() === "Finish");
  ...
  it("recovers from a failed connect attempt once messenger connect resolves", async () => {
    ...
    const step3 = wrapper.get('[data-step="step-3"]');
    const connectBtn = () =>
      step3.findAll("button").find((btn) => btn.text() === "Connect");
  ...
  it("prevents advancing when key missing and handles extension failure", async () => {
    ...
    const connectExtension = step1
      .findAll("button")
      .find((btn) => btn.text() === "Connect Extension");
...
describe("NostrIdentityManager", () => {
  ...
  it("adds and removes aliases while persisting to store", async () => {
    ...
    const openBtn = wrapper
      .findAll("button")
      .find((btn) => btn.text() === "Identity / Relays");
  ...
  it("adds and removes relays in settings", async () => {
    ...
    const relayInput = wrapper.get('input[data-label="Add Relay"]');
    await relayInput.setValue("wss://relay.test");
    await relayInput.trigger("keyup", { key: "Enter" });
    await nextTick();
...
describe("WelcomePage", () => {
  ...
  it("disables navigation when canProceed blocks the step", () => {
    ...
    const nextBtn = wrapper
      .findAll("button")
      .find((btn) => btn.text().includes("Welcome.footer.next"));
  ...
  it("finishes onboarding through footer and marks welcome seen", async () => {
    ...
    const finishBtn = wrapper
      .findAll("button")
      .find((btn) => btn.text().includes("Welcome.footer.finish"));
    expect(finishBtn).toBeTruthy();
    await finishBtn!.trigger("click");
    await flushPromises();
    ...
    expect(routerMock.replace).toHaveBeenCalledWith("/wallet");
  });
