...
vi.mock("components/DonateDialog.vue", () => ({
  default: { name: "DonateDialog", template: "<div></div>" },
}));
vi.mock("components/SubscribeDialog.vue", () => ({
  default: { name: "SubscribeDialog", template: "<div></div>" },
}));
vi.mock("components/SendTokenDialog.vue", () => ({
  default: { name: "SendTokenDialog", template: "<div></div>" },
}));
vi.mock("components/MediaPreview.vue", () => ({
  default: { name: "MediaPreview", template: "<div></div>" },
}));
...
vi.mock("stores/donationPresets", () => ({
  useDonationPresetsStore: () => ({ createDonationPreset: vi.fn() }),
}));
...
describe("FindCreators redirection", () => {
  it("redirects to creator route when npub query is present", async () => {
    ...
    expect(routerMock.replace).toHaveBeenCalledWith({
      ...
      params: { npubOrHex: "testnpub" },
    });
  });

  it("redirects to creator profile when the route param is present", async () => {
    const pinia = createPinia();
    setActivePinia(pinia);
    routerMock.replace.mockReset();
    routeMock.params = { npubOrHex: "testhexpub" };
    mount(FindCreators, {
      global: {
        plugins: [pinia],
        stubs: [
